          ...
          <Col md={8} className="ms-auto">
            <Card>
              <Card.Body>
                ...
                <Card.Text className="card-text">Email: {user.email}</Card.Text>
                <Stack direction="horizontal" gap={2}>
                  <Link to="/updateEmail" className="btn btn-link ms-2">
                    Update Email
                  </Link>
                  ...
                  {user.isAdmin && (
                    <Link to="/admin" className="btn ms-auto contrast">
                      Admin Dashboard
                    </Link>
                  )}
                </Stack>
            ...
            {orders.map((order, index) => (
              <Col key={index} md={4}>
                <Card className="my-2 shadow shadow-primary">
                  <Card.Body>
                    <Card.Title>
                      <Link className="btn contrast" to={`/order/${order._id}`}>
                        Order ID: {order._id}
                      </Link>
                      <hr className="shadow" />
                    </Card.Title>
